		...
		for _epoch in tqdm(range(train_progress.epoch, self.config.epochs, 1), desc="epoch"):
			...
			for batch in step_tqdm:
				...
				def combined_score(
					...
					"""
					Calcula um score combinado entre erro relativo (magnitude) e Cosine Similarity (direção).
					
					Args:
						predicted (torch.Tensor): Tensores preditos [batch, channels, height, width].
						target (torch.Tensor): Tensores-alvo (verdadeiros) [batch, channels, height, width].
						alpha (float, opcional): Peso para o erro relativo vs. Cosine Similarity. 
												Por padrão, 0.5 (meio a meio).
						epsilon (float, opcional): Pequeno valor para evitar divisão por zero.

					Returns:
						torch.Tensor: Vetor 1D (tamanho batch), onde cada elemento é o score combinado 
									(quanto maior, melhor a predição).
					"""
					# 1) Erro relativo médio por amostra do batch
					relative_error = torch.abs((target - predicted) / (target + epsilon))
					relative_error_mean = relative_error.mean(dim=[1, 2, 3])  # reduz por canais, H, W

					# 2) Cosine Similarity por amostra do batch
					#    Achata [channels, height, width] para uma dimensão única
					pred_flat = predicted.flatten(start_dim=1)  # [batch, channels*height*width]
					targ_flat = target.flatten(start_dim=1)     # idem
					cos_sim = F.cosine_similarity(pred_flat, targ_flat, dim=1)  # retorna [batch]

					# 3) Combinação do score
					#    (1 - relative_error_mean) faz com que 0 seja pior (erro = 1) e 1 seja melhor (erro = 0)
					#    cos_sim já está em [ -1..1 ], mas em geral ruídos podem ficar em [0..1] ou [-1..1]
					score = alpha * (1 - relative_error_mean) + (1 - alpha) * cos_sim
					
					return score


				with TorchMemoryRecorder(enabled=False):
					epoch_length = self.data_loader.get_data_set().approximate_length()
					original_threshold = 0.60  # o valor original
					threshold = original_threshold
					fail_count = 0  # falhas consecutivas

					while True:
						model_output_data = self.model_setup.predict(self.model, batch, self.config, train_progress)
						
						predicted = model_output_data.get("predicted")
						target = model_output_data.get("target")

						comb_score = combined_score(predicted, target).mean().item()

						if comb_score >= threshold:
							threshold = original_threshold  # restaurar o threshold original
							del predicted, target
							break
						else: 
							fail_count += 1
							
							# redução temporária a cada 10 fails
							if fail_count % 5 == 0:
								threshold -= 0.01								
							
							# libera memória após cada tentativa
							del model_output_data, predicted, target
							torch.cuda.empty_cache()

					# Calcula a perda atual
					...
					# Calcula a perda final levando em conta a acumulação de gradientes
					loss = loss / self.config.gradient_accumulation_steps

					# Realiza o backward pass com ou sem escalonador de gradientes
					if scaler:
