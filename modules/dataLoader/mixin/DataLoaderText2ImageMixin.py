...
from modules.util.enum.DataType import DataType
from modules.model import util as model_util  # ADDED Model util for VAE device management
...
class DataLoaderText2ImageMixin:
    ...
    def _augmentation_modules(self, config: TrainConfig):
        ...
        random_flip = RandomFlip(
            names=inputs, enabled_in_name="concept.image.enable_random_flip", fixed_enabled_in_name="concept.image.enable_fixed_flip"
        )
    ...
    def _output_modules_from_out_names(
        self,
        output_names: list[str | tuple[str, str]],  # Allow tuple for renaming
        sort_names: list[str],  # sort_names is now passed in
        config: TrainConfig,
        ...
        # Before cache fun needs VAE on train device if using GPU Cache for RandomLatentMaskRemove's potential generation
        def before_mask_remove_fun():
            if vae:
                if config.cache_mode == CacheMode.GPU:  # Check CacheMode
                    # If caching on GPU, VAE might be needed on train_device by RandomLatentMaskRemove
                    model_util.vae_to(vae, torch.device(config.train_device))  # Use device from config
                elif before_cache_image_fun:
                    # Otherwise, use the standard before_cache function (likely moves VAE to temp)
                    before_cache_image_fun()
        ...
        mask_remove = RandomLatentMaskRemove(
            ...
            before_cache_fun=before_mask_remove_fun,  # Use the adjusted function
        )

        # Batch sorting choice depends on cache mode
        # This replaces the old logic based on config.latent_caching
        if config.cache_mode == CacheMode.DISK:
            # With Disk cache, sorting happens before cache, use standard AspectBatchSorting
            batch_sorting = AspectBatchSorting(resolution_in_name="crop_resolution", names=sort_names, batch_size=config.batch_size)
        else:
            # With GPU cache or No cache, sorting happens inline after potential caching
        ...
        modules.append(batch_sorting)  # Add the appropriate batch sorter
